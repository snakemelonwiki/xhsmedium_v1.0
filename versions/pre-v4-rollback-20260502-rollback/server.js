...
function normalizeLeadProcessStatus(value, lead = {}) {
  const raw = String(value || "").trim();
  if (raw === "已接" || raw === "未接") return raw;
  if (lead.salesFeedback || ["跟进中", "已成交", "无效"].includes(String(lead.status || "").trim())) {
    return "已接";
  }
  return "未接";
}

function normalizeLeadAddStatus(value) {
  if (value === true || value === "true" || value === 1 || value === "1" || value === "已添加") return "已添加";
  return "未添加";
}

function normalizeLeadIntention(value) {
  const raw = String(value || "").trim();
  return ["强意向", "预算不够", "了解备用"].includes(raw) ? raw : "";
}

function normalizeLeadReminderLogs(items) {
  return Array.isArray(items)
    ? items
      .filter(Boolean)
      .map((item) => ({
        id: item.id || makeId("lead-reminder"),
        target: item.target === "operator" ? "operator" : "sales",
        createdAt: item.createdAt || nowIso(),
...
function normalizeLeadRecord(lead, users = []) {
  const assignedSalesUserId = lead.assignedSalesUserId || "";
  const assignedSalesUserName = lead.assignedSalesUserName
    || users.find((item) => item.id === assignedSalesUserId)?.username
    || "";
  return {
    ...lead,
    assignedSalesUserId,
    assignedSalesUserName,
    processStatus: normalizeLeadProcessStatus(lead.processStatus, lead),
    addStatus: normalizeLeadAddStatus(lead.addStatus),
    intention: normalizeLeadIntention(lead.intention),
    reminderLogs: normalizeLeadReminderLogs(lead.reminderLogs)
  };
}
...
function normalizeLocalDb(db) {
  ...
  const posts = (db.posts || []).map((post) => {
    ...
    if (postType !== post.postType || traffic !== Number(post.traffic || 0)) {
      changed = true;
    }
    return {
      ...post,
  ...
  const leads = (db.leads || []).map((lead) => {
    const normalizedLead = normalizeLeadRecord(lead, ownerUsers);
    if (
      normalizedLead.assignedSalesUserId !== (lead.assignedSalesUserId || "")
      || normalizedLead.assignedSalesUserName !== (lead.assignedSalesUserName || "")
      || normalizedLead.processStatus !== (lead.processStatus || "")
      || normalizedLead.addStatus !== (lead.addStatus || "")
      || normalizedLead.intention !== (lead.intention || "")
      || JSON.stringify(normalizedLead.reminderLogs) !== JSON.stringify(Array.isArray(lead.reminderLogs) ? lead.reminderLogs : [])
    ) {
      changed = true;
    }
...
function enrichLead(db, lead) {
  ...
  const assignedSalesUser = db.users.find((item) => item.id === lead.assignedSalesUserId && item.role === "sales");
  const normalizedLead = normalizeLeadRecord(lead, db.users || []);
  return {
    ...normalizedLead,
    employeeName: employee?.name || "",
    accountName: account?.accountName || "",
    ...
    salesFeedback: normalizedLead.salesFeedback || "",
    salesUpdatedAt: normalizedLead.salesUpdatedAt || "",
    salesUserName: normalizedLead.salesUserName || "",
    captureImageUrl: normalizedLead.captureImageUrl || "",
    assignedSalesUserId: normalizedLead.assignedSalesUserId || "",
    assignedSalesUserName: normalizedLead.assignedSalesUserName || assignedSalesUser?.username || "",
    processStatus: normalizedLead.processStatus || "未接",
    addStatus: normalizedLead.addStatus || "未添加",
    intention: normalizedLead.intention || "",
    reminderLogs: normalizedLead.reminderLogs || []
  };
}
...
function findRecentDuplicateLead(leads, targetLead, windowMs = 60000) {
  const targetSignature = getLeadDuplicateSignature(targetLead);
  const targetTime = new Date(targetLead.createdAt || nowIso()).getTime();
  return (leads || []).find((item) => {
    if (getLeadDuplicateSignature(item) !== targetSignature) return false;
    const itemTime = new Date(item.createdAt || item.updatedAt || 0).getTime();
    return Number.isFinite(itemTime) && Math.abs(targetTime - itemTime) <= windowMs;
  }) || null;
}
...
app.get("/api/leads", authRequired, async (req, res) => {
  ...
  if (isMysqlEnabled()) {
    ...
    const rows = leads.map((item) => ({
      ...
      if (req.session.role !== "sales") return true;
      return !item.assignedSalesUserId || item.assignedSalesUserId === req.session.userId;
    });
    return res.json(rows);
  }
  ...
  const rows = db.leads
    .filter((item) => !visibleEmployeeId || item.employeeId === visibleEmployeeId)
    .filter((item) => req.session.role !== "sales" || !item.assignedSalesUserId || item.assignedSalesUserId === req.session.userId)
    .map((item) => enrichLead(db, item));
  res.json(rows);
});
...
app.post("/api/leads", authRequired, upload.single("captureImage"), async (req, res) => {
  if (isMysqlEnabled()) {
    const accounts = await repositories.listAccounts();
    const existingLeads = await repositories.listLeads();
    const users = await repositories.listUsers();
    const account = accounts.find((item) => item.id === req.body.accountId);
    const employeeId = req.session.role === "staff" ? req.session.employeeId : req.body.employeeId;
    const createdAt = nowIso();
    const assignedSalesUserId = req.body.assignedSalesUserId || "";
    const assignedSalesUserName = users.find((item) => item.id === assignedSalesUserId && item.role === "sales")?.username || "";
    const row = {
      id: makeId("lead"),
      employeeId,
      accountId: req.body.accountId,
      ...
      assignedSalesUserId,
      assignedSalesUserName,
      processStatus: normalizeLeadProcessStatus(req.body.processStatus, { status: req.body.status, salesFeedback: req.body.salesFeedback }),
      addStatus: normalizeLeadAddStatus(req.body.addStatus),
      intention: normalizeLeadIntention(req.body.intention),
      reminderLogs: [],
      createdAt,
      updatedAt: createdAt
    };
    const duplicateLead = findRecentDuplicateLead(existingLeads, row);
    if (duplicateLead) {
      ...
      return res.json({
        ...duplicateLead,
        employeeName: employees.find((item) => item.id === duplicateLead.employeeId)?.name || "",
        accountName: accounts.find((item) => item.id === duplicateLead.accountId)?.accountName || "",
        sourcePostTitle: posts.find((item) => item.id === duplicateLead.postId)?.title || "",
        sourcePostUrl: posts.find((item) => item.id === duplicateLead.postId)?.postUrl || "",
        sourcePostType: posts.find((item) => item.id === duplicateLead.postId)?.postType || "",
  ...
  const db = readDb();
    ...
    const createdAt = nowIso();
    const assignedSalesUserId = req.body.assignedSalesUserId || "";
    const assignedSalesUserName = db.users.find((item) => item.id === assignedSalesUserId && item.role === "sales")?.username || "";
    const row = {
      id: makeId("lead"),
      employeeId,
    accountId: req.body.accountId,
  ...
  const duplicateLead = findRecentDuplicateLead(db.leads, row);
  if (duplicateLead) {
    return res.json({
      ...enrichLead(db, duplicateLead),
      duplicateBlocked: true
    });
  }
...
app.put("/api/leads/:id", authRequired, upload.single("captureImage"), async (req, res) => {
  if (isMysqlEnabled()) {
    ...
    const assignedSalesUserId = req.body.assignedSalesUserId !== undefined
      ? String(req.body.assignedSalesUserId || "")
      : (current.assignedSalesUserId || "");
    const assignedSalesUserName = users.find((item) => item.id === assignedSalesUserId && item.role === "sales")?.username || "";
    await repositories.updateLead({
      ...
      assignedSalesUserId,
      assignedSalesUserName,
      processStatus: normalizeLeadProcessStatus(req.body.processStatus ?? current.processStatus, {
        status: req.body.status || current.status,
        salesFeedback: req.body.salesFeedback ?? current.salesFeedback
      }),
      addStatus: normalizeLeadAddStatus(req.body.addStatus ?? current.addStatus),
      intention: normalizeLeadIntention(req.body.intention ?? current.intention),
      reminderLogs: Array.isArray(current.reminderLogs) ? current.reminderLogs : []
    });
    await persistDailySnapshotsFromRepositories();
    return res.json({ ok: true });
  }
  ...
  db.leads = db.leads.map((item) => {
    ...
    return {
      ...
      assignedSalesUserId,
      assignedSalesUserName,
      processStatus: normalizeLeadProcessStatus(req.body.processStatus ?? item.processStatus, {
        status: req.body.status || item.status,
        salesFeedback: req.body.salesFeedback ?? item.salesFeedback
      }),
      addStatus: normalizeLeadAddStatus(req.body.addStatus ?? item.addStatus),
      intention: normalizeLeadIntention(req.body.intention ?? item.intention),
      reminderLogs: normalizeLeadReminderLogs(item.reminderLogs),
      updatedAt: nowIso()
    };
  });
...
app.post("/api/leads/:id/remind", authRequired, async (req, res) => {
  const target = String(req.body.target || "sales").trim() === "operator" ? "operator" : "sales";
  if (isMysqlEnabled()) {
    const leads = await repositories.listLeads();
    const lead = leads.find((item) => item.id === req.params.id);
    if (!lead) return res.status(404).json({ message: "客资不存在" });
    const db = readDb();
    createNotification(db, {
      type: target === "sales" ? "lead_remind_sales" : "lead_remind_operator",
      title: target === "sales" ? "有客资需要销售处理" : "有客资需要运营补充",
      message: `${req.session.username || "系统"}提醒处理客资 ${lead.contactInfo || lead.nickname || ""}`.trim(),
      fromUserId: req.session.userId,
      audienceRoles: target === "sales" ? ["sales"] : ["staff", "admin", "owner"],
  ...
  if (!lead) return res.status(404).json({ message: "客资不存在" });
  const reminder = {
    id: makeId("lead-reminder"),
    target,
    createdAt: nowIso(),
    createdBy: req.session.userId,
    createdByName: req.session.username || ""
  };
  db.leads = db.leads.map((item) => item.id === req.params.id ? {
    ...item,
    reminderLogs: [reminder, ...normalizeLeadReminderLogs(item.reminderLogs)].slice(0, 10),
    updatedAt: nowIso()
  } : item);
  createNotification(db, {
    type: target === "sales" ? "lead_remind_sales" : "lead_remind_operator",
