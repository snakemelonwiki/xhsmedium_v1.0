...
async function createAccount(account) {
  await query(
    "INSERT INTO accounts (id, employee_id, platform, profile_url, account_name, account_uid, persona, positioning, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [account.id, account.employeeId, account.platform, account.profileUrl, account.accountName, account.accountUid, account.persona, account.positioning, account.status]
  );
}

async function updateAccount(account) {
  await query(
    "UPDATE accounts SET employee_id = ?, platform = ?, profile_url = ?, account_name = ?, account_uid = ?, persona = ?, positioning = ?, status = ? WHERE id = ?",
    [account.employeeId, account.platform, account.profileUrl, account.accountName, account.accountUid, account.persona, account.positioning, account.status, account.id]
  );
}
...
async function createPost(post) {
  await query(
    "INSERT INTO posts (id, employee_id, account_id, platform, title, cover_image_url, post_url, post_type, traffic, likes, comments, favorites, metrics_updated_at, published_at, note) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [post.id, post.employeeId, post.accountId, post.platform, post.title, post.coverImageUrl, post.postUrl, post.postType, post.traffic, post.likes, post.comments, post.favorites, post.metricsUpdatedAt || null, post.publishedAt, post.note]
  );
}

async function updatePost(post) {
  await query(
    "UPDATE posts SET account_id = ?, title = ?, cover_image_url = ?, post_url = ?, post_type = ?, traffic = ?, likes = ?, comments = ?, favorites = ?, metrics_updated_at = ?, published_at = ?, note = ? WHERE id = ?",
    [post.accountId, post.title, post.coverImageUrl, post.postUrl, post.postType, post.traffic, post.likes, post.comments, post.favorites, post.metricsUpdatedAt || null, post.publishedAt, post.note, post.id]
  );
}
...
async function updateLead(lead) {
  await query(
    ...
    [lead.accountId, lead.postId || null, lead.contactInfo, lead.nickname || "", lead.budget, lead.majorContent, lead.ip || "", lead.status, lead.dealAmount || null, lead.note, lead.captureImageUrl || "", lead.salesFeedback || "", lead.salesUpdatedAt || null, lead.salesUserName || "", lead.assignedSalesUserId || "", lead.assignedSalesUserName || "", lead.processStatus || "未接", lead.addStatus || "未添加", lead.intention || "", lead.id]
  );
}
